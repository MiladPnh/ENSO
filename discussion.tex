...
(Sec.~\ref{sec:skill}). Three qualifications are essential to reading that number
correctly.
...
The interpretability claim should nonetheless be bounded carefully, and the parameterisation
sets most of the bounds. The periods are initialised in-band and, more restrictively, are
confined by construction to between one and ten years (Sec.~\ref{sec:operator}); they are
therefore displaced from their starting values within an imposed range, not discovered
independently of it. Stability is imposed the same way, so the fact that all modes are damped
carries no information. Only the leading mode is dynamically excited above an isotropic
...
Section~\ref{sec:ablations} reports three controlled ablations, none of which improved skill:
...
El Ni\~no and 14 La Ni\~na episodes exceeding $1.0\dgr$C, which is why event-discrimination
intervals are wide and why the paired year-block bootstrap, rather than window-level
resampling, is used throughout. Related, the event threshold used here is $\pm1.0\dgr$C rather
