...
sub-degree structure, at a scale set by the pooling and patch sizes of the encoder
(Sec.~\ref{sec:compression}); the relevant question is not whether information is lost, but
whether the quantities the model is scored on survive the round trip.
Figure~\ref{fig:ae-fidelity} answers this directly.
...
$0.25\dgr$ grid, rising to 0.944 at $1\dgr$, 0.981 at $5\dgr$, and 0.995 at $20\dgr$ averaging.
...
ACC $0.999$, RMSE $0.047\dgr$C, amplitude ratio $0.990$; WWV reaches ACC $0.993$, RMSE
...
\begin{figure}[t]
  ...
  \caption{The latent coordinates. \textbf{(a)--(c)} SST-anomaly regression footprints
  ($\dgr$C per standard deviation of the coordinate) for the three coordinates with above-average
...
multitaper \citep{Thomson1982}, $NW=2$, $K=3$; AR(1) null matched in variance and lag-1 autocorrelation; 95\% level
...
Event thresholds in this section are $\pm1.0\dgr$C, not the conventional $\pm0.5$ ONI
...
\begin{figure}[t]
  ...
  leads 12--17 difference to the event threshold, 0.5 to $1.5\dgr$C.}
  \label{fig:event-auc}
\end{figure}
...
an artefact of the $\pm1.0\dgr$C threshold: the El Ni\~no gain is positive at every threshold
tested, 0.5 to $1.5\dgr$C, and significant from $0.625\dgr$C upward, but at the conventional
$0.5\dgr$C threshold it is $+0.054$ [$-0.015$, $+0.124$], not significant -- the model's
event-discrimination gain is specific to substantial events, and the record cannot resolve it
at the weak-event threshold. Above approximately $1.05\dgr$C the La Ni\~na difference turns
...
$|\mathrm{anomaly}|\ge1.0\dgr$C over 1981--2024, the 1992 and 2002 El Ni\~no events have no
...
The three strongest observed El Ni\~nos, all near $+2.8\dgr$C, illustrate the same shortfall
...
$+1.7\dgr$C against an observed $+2.9\dgr$C; the model then largely misses the transition into
...
cold event in the record, reaches approximately $-1.3\dgr$C against an observed $-2.5\dgr$C at
lead 6. The 2020--22 multi-year La Ni\~na is the weakest case: the lead-6 forecast is $-0.21$
against an observed $-1.40\dgr$C, and the model stays close to zero while the event persists --
...
\begin{figure}[t]
  ...
  over the equatorial waveguide ($5\dgr$S--$5\dgr$N, $160\dgr$E--$80\dgr$W), against lead, for
